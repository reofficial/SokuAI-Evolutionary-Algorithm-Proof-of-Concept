...
class GeneticAI:
    def __init__(self, population_size=100,  # Increased population
                 ...
                 output_size=10,
                 hidden_layers=[32,32,32,32,32,32,32],  # Deeper architecture
                 activation_sequence=['tanh', 'sin', 'relu', 'tanh', 'sin', 'relu', 'tanh'],
                 mutation_rate=0.50):
...
def train():
    ...
    ai = GeneticAI(population_size=10)
    ...
    try:
        while True:                
            ...
            fitness = []            
            for idx, p1 in enumerate(ai.population):                 
                
                controller_p1 = InputController(2)
                controller_p2 = InputController(1)  # Non-AI controlled
                ...
                while time.time() - start_time < 15:
                    ...
                    # Replace with non-AI behavior for Player 2 (e.g., random actions)
                    #actions_p2 = [np.random.rand() > 0.5 for _ in range(len(actions_p1))]  # Example
                ...
                try:
                    ai.fitness_scores[idx] = calculate_fitness(
                        health_history, action_history_p1, True
                    )
                    print(f"\nMatch {idx+1} Results:")
                    print(f"P1 Fitness: {ai.fitness_scores[idx]:.2f}")
                    fitness.append(ai.fitness_scores[idx].item())
                except Exception as e:
                    print(f"Error calculating fitness: {str(e)}")
                    ai.fitness_scores[idx] = 0.001
